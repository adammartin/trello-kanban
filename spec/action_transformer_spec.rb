...
describe ActionTransformer do
  ...
  let(:data) { { 'listAfter' => list, 'card' => { 'id' => card_id } } }
  let(:transformer) { ActionTransformer.new }

  before(:each) {
    give(action).id { id }
    give(action).type { type }
    give(action).data { data }
  ...
  context ', when the data is not a column move,' do
    let(:data) { { 'list' => list, 'card' => { 'id' => card_id } } }

    it 'will return nil for action' do
      expect(transformer.transform action).to be nil
    end
  end
end
